...
class CordappPlugin @Inject constructor(
    ...
    override fun apply(project: Project) {
        ...
        with(project.configurations) {
            createImplementationConfiguration(CORDAPP_CONFIGURATION_NAME)
            createImplementationConfiguration(CORDA_PROVIDED_CONFIGURATION_NAME)
            createRuntimeOnlyConfiguration(CORDA_RUNTIME_ONLY_CONFIGURATION_NAME)
            create(CORDA_CORDAPP_CONFIGURATION_NAME) {
                it.attributes { attr ->
    ...
    private fun configurePomCreation(project: Project) {
        with(project.tasks) {
            withType(GenerateMavenPom::class.java).configureEach { task ->
                task.doFirst {
            ...
            withType(GenerateModuleMetadata::class.java).configureEach { task ->
